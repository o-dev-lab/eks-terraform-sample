...
locals {
  node_iam_role_arns_non_windows = distinct(
    compact(
      concat(
        [for group in var.eks_managed_node_group : group.iam_role_arn],
        [for group in var.self_managed_node_group : group.iam_role_arn if group.platform != "windows"],
  ...
  fargate_profile_pod_execution_role_arns = distinct(
    compact(
      concat(
        [for group in var.fargate_profile : group.fargate_profile_pod_execution_role_arn],
